...
\moduleref{$if(moduleref)$$moduleref$$endif$}
\modulenom{$if(modulenom)$$modulenom$$endif$}
$if(enseig)$
  \enseig{$for(enseig)$$enseig$$sep$, $endfor$}
$endif$
$if(email)$
...
\AddToShipoutPictureBG*{\includegraphics[width=\paperwidth,height=\paperheight]{$if(backgroundimage)$$backgroundimage$$else$FondURCA.png$endif$}}
