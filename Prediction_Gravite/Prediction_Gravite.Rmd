...
La méthode XGBoost donne un taux d'erreur de `r round(err_xg,2)`%, on obtient dans un modèle prédictif donnant `r 1 - round(err_xg,2)`% de bonnes prédictions.  
...
La méthode Random Forest donne un taux d'erreur de `r round(err_rf,2)`%, on obtient dans un modèle prédictif donnant `r 1 - round(err_rf,2)`% de bonnes prédictions.  
On représente la répartition des bonnes et mauvaises prédictions sur le graphe suivant.  
